...
class StockEditController extends GetxController with LoadingStateMixin<StockData> {
  ...
  void onInit() {
    super.onInit();
    title = LocaleKeys.addParam.trArgs([LocaleKeys.stock.tr]);
    final params = Get.parameters;
    if (params.isNotEmpty) {
      id = params['id'];
      if (id != null) {
        title = LocaleKeys.editParam.trArgs([LocaleKeys.stock.tr]);
      }
    }
    addOrEdit();
  }
  ...
  Future<void> addOrEdit() async {
    ...
    oldRow = null;
    try {
      final DioApiResult dioApiResult = await apiClient.get(Config.stockAddOrEdit, data: {'id': id});
      ...
      hasPermission = true;
      final resultModel = stockEditModelFromJson(dioApiResult.data);
  ...
  Future<void> save() async {
    ...
    if (formKey.currentState?.saveAndValidate() ?? false) {
      final preCtl = Get.find<StockController>();
      ...
      try {
        ...
        switch (data["status"]) {
          case 200:
            ...
            final resultModel = StockData.fromJson(apiResult);
            if (id == null) {
              preCtl.dataList.insert(0, resultModel);
            } else {
              final index = preCtl.dataList.indexWhere((element) => element.tStockId.toString() == id);
              if (index != -1) {
