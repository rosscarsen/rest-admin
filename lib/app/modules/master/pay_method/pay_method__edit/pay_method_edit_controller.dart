...
class PayMethodEditController extends GetxController with LoadingStateMixin<PayMethodEditResult> {
  ...
  String? id;
  //编辑前的数据
  PayMethodData? oldRow;

  @override
  void onInit() {
    super.onInit();
    title = LocaleKeys.addParam.trArgs([LocaleKeys.paymentMethod.tr]);
    final params = Get.parameters;
    if (params.isNotEmpty) {
      id = params['id'];
      if (id != null) {
        title = LocaleKeys.editParam.trArgs([LocaleKeys.paymentMethod.tr]);
      }
    }
    addOrEdit();
  }
  ...
  Future<void> addOrEdit() async {
    ...
    try {
      final DioApiResult dioApiResult = await apiClient.get(
        Config.paymentMethodAddOrEdit,
        data: {'id': id, "lang": (Get.locale?.toString() ?? "zh_HK").toLowerCase()},
      );
      ...
      hasPermission = true;
      final resultModel = payMethodEditModelFromJson(dioApiResult.data);
      ...
      if (resultModel.status == 200 && resultModel.apiResult != null) {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          ...
          jsonMap.remove("connectList");
          jsonMap.remove("networkPayMethod");
  ...
  Future<void> save() async {
    ...
    if (formKey.currentState?.saveAndValidate() ?? false) {
      final preCtl = Get.find<PayMethodController>();
      ...
      final requestData = {...formData, "id": id, "lang": (Get.locale?.toString() ?? "zh_HK").toLowerCase()};
      try {
        final DioApiResult dioApiResult = await apiClient.post(Config.paymentMethodSave, data: requestData);
        if (!dioApiResult.success) {
          CustomDialog.errorMessages(dioApiResult.error ?? LocaleKeys.unknownError.tr);
          return;
        }
        ...
        switch (data["status"]) {
          case 200:
            ...
            final resultModel = PayMethodData.fromJson(apiResult);
            if (id == null) {
              preCtl.data?.insert(0, resultModel);
            } else {
              final index = preCtl.data?.indexWhere((element) => element.tPayTypeId.toString() == id) ?? -1;
              if (index != -1) {
            ...
            break;
          case 201:
            CustomDialog.errorMessages(LocaleKeys.codeExists.trArgs([LocaleKeys.paymentMethod.tr]));
            break;
          case 202:
