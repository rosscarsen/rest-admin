...
import '../../../model/department/department_page_model.dart';
...
class DepartmentController extends GetxController with LoadingStateMixin<List<DepartmentData>?> {
  final GlobalKey<FormBuilderState> formKey = GlobalKey<FormBuilderState>();
  final ApiClient apiClient = ApiClient();
  late DepartmentDataSource dataSource;
  @override
  void onInit() {
    super.onInit();
  ...
  void updateDataGridSource() {
    getList().then((_) {
      dataSource = DepartmentDataSource(this);
    });
  }
  ...
  Future<void> getList() async {
    ...
    try {
      ...
      final DioApiResult dioApiResult = await apiClient.get(Config.department, data: param);
      ...
      final resultModel = departmentPageModelFromJson(dioApiResult.data.toString());
  ...
  void edit({String? id}) async {
    Get.toNamed(Routes.DEPARTMENT_EDIT, parameters: id == null ? null : {'id': id.toString()});
  }
    ...
    CustomAlert.iosAlert(
      ...
      onConfirm: () async {
        try {
          ...
          final DioApiResult dioApiResult = await apiClient.get(Config.departmentDelete, data: {"id": id});
          ...
          switch (apiData['status']) {
            case 200:
              ...
              data?.removeWhere((element) => element.tBrandId == id);
