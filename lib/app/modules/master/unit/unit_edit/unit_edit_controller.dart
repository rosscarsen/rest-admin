...
import 'model/unit_edit_model.dart';

class UnitEditController extends GetxController with LoadingStateMixin<UnitData> {
  ...
  void onInit() {
    super.onInit();
    title = LocaleKeys.addParam.trArgs([LocaleKeys.unit.tr]);
    final params = Get.parameters;
    if (params.isNotEmpty) {
      id = params['id'];
      if (id != null) {
        title = LocaleKeys.editParam.trArgs([LocaleKeys.unit.tr]);
      }
    }
    addOrEdit();
  }
  ...
  Future<void> addOrEdit() async {
    ...
    oldRow = null;
    try {
      final DioApiResult dioApiResult = await apiClient.get(Config.unitAddOrEdit, data: {'id': id});
      ...
      hasPermission = true;
      final resultModel = unitEditModelFromJson(dioApiResult.data);
  ...
  Future<void> save() async {
    ...
    if (formKey.currentState?.saveAndValidate() ?? false) {
      final preCtl = Get.find<UnitController>();
      ...
      try {
        final DioApiResult dioApiResult = await apiClient.post(Config.unitSave, data: requestData);
        ...
        switch (data["status"]) {
          case 200:
            ...
            final resultModel = UnitData.fromJson(apiResult);
            if (id == null) {
              preCtl.dataList.insert(0, resultModel);
            } else {
              final index = preCtl.dataList.indexWhere((element) => element.tUnitId.toString() == id);
              if (index != -1) {
            ...
            break;
          case 201:
            CustomDialog.errorMessages(LocaleKeys.codeExists.trArgs([LocaleKeys.unit.tr]));
            break;
          case 202:
