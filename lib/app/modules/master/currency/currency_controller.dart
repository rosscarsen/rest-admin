...
import '../../../model/currency/currency_page_model.dart';
...
import 'currency_data_source.dart';

class CurrencyController extends GetxController with LoadingStateMixin {
  final GlobalKey<FormBuilderState> formKey = GlobalKey<FormBuilderState>();
  List<CurrencyData> dataList = [];
  final ApiClient apiClient = ApiClient();
  late CurrencyDataSource dataSource;
  @override
  void onInit() {
    super.onInit();
  ...
  void updateDataGridSource() {
    getList().then((_) {
      dataSource = CurrencyDataSource(this);
    });
  }
  ...
  Future<void> getList() async {
    ...
    try {
      ...
      final DioApiResult dioApiResult = await apiClient.get(Config.currency, data: param);
      ...
      final resultModel = currencyPageModelFromJson(dioApiResult.data.toString());
  ...
  void edit({String? id}) async {
    Get.toNamed(Routes.CURRENCY_EDIT, parameters: id == null ? null : {'id': id.toString()});
  }
    ...
    CustomAlert.iosAlert(
      ...
      onConfirm: () async {
        try {
          ...
          final DioApiResult dioApiResult = await apiClient.get(Config.currencyDelete, data: {"id": id});
          ...
          switch (data['status']) {
            case 200:
              ...
              dataList.removeWhere((element) => element.tMoneyCurrencyId == id);
