...
class NetWorkPayMethodEditController extends GetxController with LoadingStateMixin<NetworkPayMethodEditResult> {
  ...
  String? id;
  //编辑前的数据
  NetworkPayMethodData? oldRow;

  @override
  void onInit() {
    super.onInit();
    title = LocaleKeys.addParam.trArgs([LocaleKeys.networkPayMethod.tr]);
    final params = Get.parameters;
    if (params.isNotEmpty) {
      id = params['id'];
      if (id != null) {
        title = LocaleKeys.editParam.trArgs([LocaleKeys.networkPayMethod.tr]);
      }
    }
    addOrEdit();
  }
  ...
  Future<void> addOrEdit() async {
    ...
    oldRow = null;
    try {
      final DioApiResult dioApiResult = await apiClient.get(Config.networkPaymentMethodAddOrEdit, data: {'id': id});
      ...
      hasPermission = true;
      final resultModel = networkPayMethodEditModelFromJson(dioApiResult.data);
  ...
  Future<void> save() async {
    ...
    if (formKey.currentState?.saveAndValidate() ?? false) {
      final preCtl = Get.find<NetworkPayMethodController>();
      ...
      try {
        ...
        switch (data["status"]) {
          case 200:
            ...
            final resultModel = NetworkPayMethodData.fromJson(apiResult);
            if (id == null) {
              preCtl.data?.insert(0, resultModel);
            } else {
              final index = preCtl.data?.indexWhere((element) => element.id.toString() == id) ?? -1;
              if (index != -1) {
            ...
            break;
          case 201:
            CustomDialog.errorMessages(LocaleKeys.codeExists.trArgs([LocaleKeys.networkPayMethod.tr]));
            break;
          case 202:
